...
namespace libsgp4
{
    ...
    void Tle::Initialize()
    {
        ...
        {
            std::cout << "line_one_ is the wrong length" << line_one_.length() << "\n";
            assert(IsValidLineLength(line_one_));
        }

        if (!IsValidLineLength(line_two_))
        {
            std::cout << "line_two_ is the wrong length" << line_two_.length() << "\n";
            assert(IsValidLineLength(line_two_));
        }

        if (line_one_[0] != '1')
        {
            assert((line_one_[0] == '1'));
        }

        if (line_two_[0] != '2')
        {
            assert((line_two_[0] == '2'));
        }
        ...
        {
            assert(sat_number_1 == sat_number_2);
        }
    ...
    void Tle::ExtractDouble(const std::string &str, int point_pos, double &val)
    {
        ...
        {
            assert(Util::FromString<double>(temp, val));
        }
    }
